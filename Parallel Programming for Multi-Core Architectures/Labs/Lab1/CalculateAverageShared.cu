...
__global__ void calculateAvg(int *data, double *avg, int N) 
{
    ...
    shared[tid] = (idx < N) ? (double)data[idx]/N : 0.0;
    __syncthreads();

    // Redukcja równoległa (trochę jak merge sort)
...
int main(int argc,char **argv) 
{
    ...
    calculateAvg<<<blocksingrid, threadsinblock, threadsinblock * sizeof(double)>>>(randomNumbersDevice, avgDevice, N);
